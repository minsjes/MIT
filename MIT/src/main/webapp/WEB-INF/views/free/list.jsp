...
<div id="layoutSidenav_content">
	<main>
		<div class="container-fluid px-4">
			<h1 class="mt-4" style='text-align: center; margin-bottom: 30px;'>자유
				게시판</h1>

			<div class="card mb-4">
				<div class="card-header">
					<span style="vertical-align: middle;"> <i
						class="bi bi-chat-square-text"></i> 자유 게시판 목록
					</span>
					...
					<table id="datatablesSimple">
						...
						<tbody>
							<c:forEach items="${list}" var="freeVO" varStatus="status">
								<tr>
									<td>${freeVO.rnum}</td>
									<td><a href='/free/read?freeNo=${freeVO.freeNo}'>
											${freeVO.freeTitle} </a></td>
									<!-- <td>${freeVO.memberName}</td> -->
									<td><c:if test="${0 eq freeVO.hiddenStatus}">
      ${freeVO.memberName}
   </c:if> <c:if test="${1 eq freeVO.hiddenStatus}">
      익명
   </c:if></td>
									<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
											value="${freeVO.freeDate}" /></td>
