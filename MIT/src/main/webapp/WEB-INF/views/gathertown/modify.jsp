...
<div id="layoutSidenav_content">
	<main>
		<div class="container-fluid px-4">
			<h1 class="mt-4" style="text-align: center; margin-bottom: 30px;">스터디 그룹 수정하기</h1>
			
			<form role="form" action="modify" method="post" name="form">
				<input type="hidden" id="gathertownNo" name="gathertownNo" value="${gathertownVO.gathertownNo}">
				<input type="hidden" id="memberNo" name="memberNo" value="${gathertownVO.memberNo}"> 
				
				<div class="row g-0">
					<div class="col-9">
						<div class="input-group">
							<span class="input-group-text" id="inputGroup-sizing-default">스터디 그룹명</span>
							<input type="text" class="form-control" id="groupName" name="groupName" value="${gathertownVO.groupName}">
						</div>
					</div>
					...
					<div class="col-3">
						<div class="input-group">
							...
							<input type="number" class="form-control" id=groupNo name="groupNo" value="${gathertownVO.groupNo}">
						</div>
					</div>
				</div>
				...
				<div class="input-group">
					...
					<input type="text" class="form-control" id="gathertownLink" name="gathertownLink" value="${gathertownVO.gathertownLink}">
				</div>
				
				<div class="form-group">
					<div class="input-group-text">스터디 그룹소개</div>
					<textarea class="form-control" id="groupContent" name="groupContent" rows="10">${gathertownVO.groupContent}</textarea>
				</div>
				...
				<c:if test="${!empty gathertownVO.thumbnail}">
					<c:set var="fileName" value="${gathertownVO.thumbnail}" />
					<c:set var="fileNo" value="${fn:toLowerCase(thumbnail)}" />
				</c:if>
...
<script>
	$(document).ready(function() {
		...
		$(".btn-outline-danger").on("click", function() {
			formObj.attr("action", "/gathertown/remove");
