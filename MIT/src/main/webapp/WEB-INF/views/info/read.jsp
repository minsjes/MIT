		...
		<div class="container-fluid px-4">
			...
			<form role="form" action="modify" method="post">
				...
				<div class="row g-0">
					...
					<div class="col">
						<div class="input-group">
							...
							<input type="text" class="form-control" value="${infoVO.infoDate}" readonly="readonly">
						</div>
					</div>
					<div class="col">
						<div class="input-group">
							<div class="input-group-text">조회수</div>
							<input type="text" class="form-control" value="${infoVO.infoView}" readonly="readonly">
						</div>
					</div>
				</div>
					...
					<div class="col-lg-2">
						<div class="input-group">
							...
							<c:if test="${1 eq infoVO.infoClass}">
								<input type="text" class="form-control" value="진로" readonly="readonly">
							</c:if>
							<c:if test="${2 eq infoVO.infoClass}">
								<input type="text" class="form-control" value="취업" readonly="readonly">
							</c:if>
							<c:if test="${9 eq infoVO.infoClass}">
								<input type="text" class="form-control" value="기타" readonly="readonly">
							</c:if>
						 </div>
					</div>
				...
				<div style="text-align: right; margin: 17px 0 17px 0;">
					...
					<c:if test="${12345678 eq login.memberNo && 12345678 ne infoVO.memberNo}">
						<button type="button" class="btn btn-outline-danger">삭제</button>
					</c:if>
...
<script>
	...
	$.getJSON("/infoComment/all/" + infoNo, function(data) {
		var str = "";
		$(data).each(function() {
			var strbutton = "";
