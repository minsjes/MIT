		...
		<div class="container-fluid px-4">
			...
			<form role="form" action="modify" method="post" name="frm">
				<input type='hidden' id="photoNo" name='photoNo'
					value="${photoVO.photoNo}"> <input type='hidden'
					id="memberNo" name='memberNo' value="${photoVO.memberNo}">
					...
					<div class="col" style="padding-right: 0;">
						<div class="input-group">
							...
							<input type="text" class="form-control" name="memberName"
								value="${photoVO.memberName}" readonly="readonly">
						</div>
					</div>
					<div class="col" style="padding-left: 0;">
						<div class="input-group">
							...
							<input type="text" class="form-control"
								value="<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${photoVO.photoDate}"/>"
								readonly="readonly">
						</div>
					</div>
				</div>
				<div class="input-group">
					...
					<input type="text" class="form-control" name='photoTitle'
						value="${photoVO.photoTitle}">
				</div>
				<div class="form-group">
					<div class="input-group-text" style="padding-left: 20px;">내용</div>
					<textarea class="form-control" name="photoContent" rows="15">${photoVO.photoContent}</textarea>
				</div>
				...
				<div class="card">
					<ul class="dropzone-previews clearfix uploadedList preview-list">
						<c:forEach items="${photoFileVO}" var="fVo" varStatus="status">
				...
				<div style="text-align: right; margin: 17px 0 17px 0;">
					...
					<button type="button" class="btn btn-outline-dark"
						onclick="location.href='/photo/list'">목록</button>
				</div>
			</form>
		</div>
	</main>
...
<a href="{{fullName}}" class="btn btn-default btn-xs pull-right delbtn"><i class="far fa-trash-alt"></i></a>
	...
	<script>
		...
		$(".fileDrop").on("drop", function(event) {
			...
			var pattern =checkImageType(file.name) ;
			if (pattern !="jpg" && pattern !="png" && pattern !="gif" && pattern !="bmp" && pattern !="jpeg" )
			{
				alert('첨부파일은 이미지 파일만 업로드할 수 있습니다.');
