	...
	<div class="container-fluid px-4">
		...
		<div class="card mb-4">
			<div class="card-header">
				<span style="vertical-align: middle;"> <i class="bi bi-question-diamond"></i> 질문 게시판 목록 </span>
				...
				<table id="datatablesSimple">
					...
					<tbody>
						<c:forEach items="${list}" var="qnaVO" varStatus="status">
							<tr>
								<td>${qnaVO.rnum}</td>
								<td><c:if test="${0 eq qnaVO.qnaClass}"> ${qnaVO.memberName}
                        </c:if> <c:if test="${1 eq qnaVO.qnaClass}"> 동아리 </c:if> 
                        <c:if test="${2 eq qnaVO.qnaClass}"> 코딩오류 </c:if> 
                        <c:if test="${3 eq qnaVO.qnaClass}"> 전공과목 </c:if> 
                        <c:if test="${4 eq qnaVO.qnaClass}"> 학교생활 </c:if></td>
								<td><a href='/qna/read?qnaNo=${qnaVO.qnaNo}'>
										${qnaVO.qnaTitle} </a></td>
								<td><c:if test="${0 eq qnaVO.hiddenStatus}"> ${qnaVO.memberName}</c:if> 
								<c:if test="${1 eq qnaVO.hiddenStatus}"> 익명 </c:if></td>

								<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
										value="${qnaVO.qnaDate}" /></td>
										<td>${qnaVO.qnaView}</td>
