		...
		<div class="container-fluid px-4">
			...
			<form role="form" action="modify" method="post" name="frm">
				...
				<div class="row g-0">
						...
						<div class="input-group">
						  ...
						  <select class="form-select" id="studyClass" name="studyClass">
                     		...
                     		<c:if test="${2 eq studyVO.studyClass}">
							    ...
							    <option value="2" selected>파이썬 기초</option>
							</c:if>
						  </select>
						</div>
					</div>
					...
					<div class="col-lg-10">
						<div class="input-group">
							...
							<input type="text" class="form-control" id="studyTitle" name="studyTitle" value="${studyVO.studyTitle}">
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="studyContent" name="studyContent">${studyVO.studyContent}</textarea>
					<script>
				...
				<div class="card">
					<ul class="dropzone-previews clearfix uploadedList preview-list">
						<c:forEach items="${studyFileVO}" var="fVo" varStatus="status">
...
<script>
	...
	function validate() {
		...
		var studyTitle = $("#studyTitle").val();
		var studyContent = CKEDITOR.instances.studyContent.getData();
		
		if (studyClass == "") {
			alert("유형을 선택해주세요.");
			document.getElementById("studyClass").focus();
			return false;
		}
