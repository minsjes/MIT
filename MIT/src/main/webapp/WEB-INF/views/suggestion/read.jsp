	...
	<main>
		<div class="container-fluid px-4">
			...
			<form role="form" action="modify" method="post">
				<input type='hidden' id="suggestionNo" name="suggestionNo" value="${suggestionVO.suggestionNo}">
				<input type='hidden' id="memberNo" name="memberNo" value="${suggestionVO.memberNo}">
				...
				<div class="row g-0">
					...
					<div class="col">
						<div class="input-group">
							...
							<input type="text" class="form-control" value="${suggestionVO.suggestionDate}" readonly="readonly">
						</div>
					</div>
				</div>
				...
				<div class="input-group">
					...
					<input type="text" class="form-control" value="${suggestionVO.suggestionTitle}" readonly="readonly">
				</div>

				<div class="form-group">
					<div class="input-group-text">&nbsp;내용</div>
					<textarea class="form-control" id="suggestionContent" readonly="readonly">${suggestionVO.suggestionContent}</textarea>
					<script>
				...
				<c:if test="${!empty suggestionFileVO}">
					<div class="input-group">
						<div class="input-group-text" style="width: 100%;">첨부파일</div>
					</div>
					<div class="card">
						<ul class="dropzone-previews preview-list">
							<c:forEach items="${suggestionFileVO}" var="suggestionFileVO" varStatus="status">
								<c:set var="fileName" value="${suggestionFileVO.fileName}" />
									...
									<div class="card mt-1 mb-0 shadow-none border dz-processing dz-image-preview dz-success dz-complete">
										<div class="p-2">
											<div class="row align-items-center px-3">
												...
												<div class="col pl-0">
													<a href="/displayFile?fileName=${suggestionFileVO.fileLocation}" text-muted font-weight-bold data-dz-name="">
														${suggestionFileVO.fileName}
													</a>
												</div>
											</div>
										</div>
				...
				<c:if test="${empty suggestionFileVO}"></c:if>

				<div style="text-align: right; margin: 17px 0 17px 0;">
					<c:if test="${login.memberNo eq suggestionVO.memberNo}">
						<button type="button" class="btn btn-outline-primary btn-modify">수정</button>
						<button type="button" class="btn btn-outline-danger">삭제</button>
					</c:if>
					<c:if test="${12345678 eq login.memberNo && 12345678 ne suggestionVO.memberNo}">
						<button type="button" class="btn btn-outline-danger">삭제</button>
					</c:if>
			...
			<div class="card mb-5">
				<div class="card-body">
					<form>
						<c:if test="${12345678 eq login.memberNo || suggestionVO.memberNo eq login.memberNo}">
							...
							<div class="text-right">
								...
								<div class="btn-group" style="float: right;">
									<a class="btn btn-outline-primary btn-rounded comentAddBtn">답변 등록</a>
								</div>
							</div>
							<br><hr>
						</c:if>
...
<script>
	$(document).ready(function() {
		...
		$(".btn-modify").on("click", function() {
			formObj.attr("action", "/suggestion/modify");
		...
		$(".btn-outline-dark").on("click", function() {
			formObj.attr("action", "/suggestion/list");
			formObj.attr("method", "get");
			formObj.submit();
		});
	});
...
<script>
	...
	$.getJSON("/suggestionComment/all/" + suggestionNo, function(data) {
		var str = "";
		$(data).each(function() {
			var strbutton = "";
			var strname = "";
	...
	function deleteReply(commentNo) {
		$.ajax({
			type : 'delete',
			url : '/suggestionComment/' + commentNo,
			headers : {
				"Content-Type" : "application/json",
